...
function meta(token) {
  ...
  if(author) {
    this.stream.write(this.format.meta(author, this.conf.AUTHOR));
    this.newline();
  }

  if(version) {
    this.stream.write(this.format.meta(version, this.conf.VERSION));
    this.newline();
  }

  if(since) {
    this.stream.write(this.format.meta(since, this.conf.SINCE));
    this.newline();
  }

  if(license) {
    this.stream.write(this.format.meta(license, this.conf.LICENSE));
    this.newline();
  }

  if(todo) {
    this.newline();
    this.stream.write(this.format.getTodo(todo)); 
  }
