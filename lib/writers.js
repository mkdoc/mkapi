 ...
 */
function meta(token) {
  var deprecated = this.tags.find(conf.DEPRECATED, token)
    , author = this.tags.find(conf.AUTHOR, token)
    , version = this.tags.find(conf.VERSION, token)
    , since = this.tags.find(conf.SINCE, token)
